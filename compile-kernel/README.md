...
Pre-compiled kernels are available in the [Releases](https://github.com/ezlp/kernel/releases) of [ezlp/kernel](https://github.com/ezlp/kernel).
...
| -f        | configFlavor | Specifies which configuration file `config-*` to download from the kernel repository [ezlp/kernel](https://github.com/ezlp/kernel/tree/main/kernel-config/release) to the local [tools/config](tools/config) directory. If a configuration file matching the kernel version already exists locally and this parameter is not set, the download is skipped. Available options correspond to the [directory names](https://github.com/ezlp/kernel/tree/main/kernel-config/release) in the kernel repository, e.g., `stable`. Default: `stable`. |
...
💡 Tip: We recommend using the kernel source code from repositories such as [linux-6.1.y](https://github.com/unifreq/linux-6.1.y), [linux-5.15.y](https://github.com/unifreq/linux-5.15.y), [linux-5.10.y](https://github.com/unifreq/linux-5.10.y) of `unifreq` for compilation, who added drivers and patches for related boxes. It is recommended to use the template in [ezlp/kernel](https://github.com/ezlp/kernel/tree/main/kernel-config/release), which has been pre-configured according to the related boxes and can be customized based on this.
